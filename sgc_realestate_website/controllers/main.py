...
class RealEstateWebsiteController(http.Controller):
    ...
    @http.route(['/', '/realestate', '/realestate/home'], type='http', auth='public', website=True, sitemap=True)
    def homepage(self, **kwargs):
        """Public homepage — featured properties + country overview"""
        try:
            Property = request.env['sgc.realestate.property'].sudo()
            Country = request.env['sgc.realestate.destination.country'].sudo()
            featured_properties = Property.search(
            ...
            stats = {
                ...
                'property_count': Property.search_count([('website_published', '=', True)]),
            }
            return request.render('sgc_realestate_website.homepage', {
                'featured_properties': featured_properties,
                'countries': countries,
                'stats': stats,
            })
        except Exception as e:
            _logger.error("Error in homepage: %s", str(e), exc_info=True)
